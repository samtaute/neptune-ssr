...
export async function fetchRawDaily(
  ...
  try {
    const response = await fetch(requestUrl, {cache:"no-cache"}); //https://nextjs.org/docs/app/building-your-application/caching
  ...
  return rawItems.map((item) => {
    ...
    return {
      ...
      squareImage: squareImages[0]?.link
        ...
        : missingImage(item),
      link: item.link,
      sourceLink: item.sourceLink ? item.sourceLink : item.link,  
      uid: item.uid,
      type: item.lbtype,
